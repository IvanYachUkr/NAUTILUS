...
import {
  availableEvidenceCues,
  availableOverviewCases,
...
test("visual evidence remains available when every included cue is non-spatial", () => {
  const run = { benchmarkId: "text-only-run", model: "Test model" };
  const caseItem = {
    clueSets: [{
      benchmarkId: "text-only-run",
      cues: [
        { id: "text-cue", annotationStatus: "text-only" },
        { id: "unboxed-cue", annotationStatus: "not-grounded" },
        { id: "excluded-cue", annotationStatus: "excluded" },
      ],
    }],
  };

  assert.deepEqual(
    availableEvidenceCues(caseItem, run).map((cue) => cue.id),
    ["text-cue", "unboxed-cue"],
  );
  assert.ok(appSource.includes("caseItem && run && clues.length && !evidenceMode"));
  assert.ok(appSource.includes("sceneClueCount.textContent = String(clues.length)"));
  assert.ok(appSource.includes("evidenceCount.textContent = String(items.length)"));
  assert.ok(appSource.includes("availableEvidenceCues(caseItem, item).length"));
});
...
test("map overview only includes cases with a prediction for the selected run slice", () => {
  ...
  assert.deepEqual(
    availableOverviewCases(cases, "Grok 4.6 · xhigh", "interactive-panorama")
      .map((caseItem) => caseItem.id),
    ["matching", "missing-pin"],
  );
  assert.ok(appSource.includes("renderLocationList(overviewCases)"));
  assert.ok(appSource.includes("renderMapHeader(caseItem, run, overviewCases, overviewRuns)"));
  assert.ok(!appSource.includes("Awaiting run"));
...
test("detail utilities keep statistics in the top-right overlay and use a count badge", () => {
  ...
  assert.ok(appSource.includes("compareModelsButton.classList.toggle(\"has-count\", comparedModelCount > 0)"));
  assert.ok(appSource.includes("compareModelsCount.textContent = comparedModelCount ? String(comparedModelCount) : \"\""));
  assert.ok(appSource.includes("compareConditionsButton.classList.toggle(\"has-count\", conditionCount > 0)"));
  assert.ok(appSource.includes("compareConditionsCount.textContent = conditionCount ? String(conditionCount) : \"\""));
  assert.ok(!appSource.includes('` · ${comparisonRuns.length}`'));
  assert.ok(!appSource.includes('` · ${conditionCount}`'));
  assert.match(expeditionStylesSource, /\.compare-models-button\.has-count\s*\{[^}]*position:\s*relative;/s);
  assert.match(expeditionStylesSource, /\.compare-models-button b:not\(:empty\)\s*\{[^}]*position:\s*absolute;/s);
...
test("long desktop location summaries can scroll without changing the mobile document flow", () => {
  assert.match(
    expeditionStylesSource,
    /@media \(min-width: 681px\)[\s\S]*?data-view-state="detail"[^}]*\.scene-story\s*\{[^}]*overflow-y:\s*auto;/s,
  );
  assert.match(expeditionStylesSource, /\.scene-story\s*\{[^}]*overscroll-behavior-y:\s*contain;/s);
});
...
test("covered static images are selectable and support same-model condition comparison", () => {
  ...
  assert.ok(appSource.includes("elements.conditionSelect.disabled = conditions.length <= 1;"));
});
